...
from ..mamba_zoo.mambaint import mamba_init
from ..ssm_zoo.ssm_methods import selective_scan_fn
from ..sub_blocks import LayerNorm2d,Linear2d,Permute
from ..ssm_zoo.crossm_methods import cross_scan_fn,cross_merge_fn
...
class SS2Dv2:
    def __initv2__(self,
        ...
        self.out_norm, forward_type = self.get_outnorm(forward_type, self.d_inner, channel_first)#Layernorm
        ...
        FORWARD_TYPES = dict(
            ...
            v05=partial(self.forward_corev2, force_fp32=False, no_einsum=True),#use
            ...
            v052dc=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="cascade2d"),#use
    ...
    def forward_corev2(self,x,force_fp32=False,no_einsum=False,scan_mode = "cross2d",):
        ...
        delta_softplus = True
        #训练期间启用float32
        force_fp32 = (self.training and (not self.disable_force32)) if force_fp32 is None else False
    ...
    def scan_rowcol(x,proj_weight,proj_bias,dt_weight,dt_bias,R,N,_As,_Ds,
        ...
        Ds = _Ds.view(-1).to(torch.float)#2d_in
